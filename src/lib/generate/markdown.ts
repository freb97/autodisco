...
function generateEntrySection(config: ParsedDiscoverConfig, schemaResults: SchemaResult[]) {
  const withTimestamp = typeof config.generate?.markdown === 'object'
    ? config.generate.markdown.timestamp !== false
    : true

  return [
    header('API Documentation'),
    paragraph(`This documentation was generated by [AutoDisco](https://github.com/bussmann-io/autodisco).`),
    list([
      `API Url: ${config.baseUrl ?? schemaResults.at(0)?.path ?? ''}`,
      ...(withTimestamp ? [`Generated on: ${new Date().toISOString()}`] : []),
    ]),
  ]
}
